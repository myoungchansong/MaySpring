...
@import url('https://fonts.googleapis.com/css?family=Nanum+Gothic');
	body{
		...
		background-image: url('${path}/img/001.jpg');
...
<script type="text/javascript">
	...
	$("#input_ph").blur(function() {
		...
		if(idVal ==""){
			$(this).css("font-size", "5px");
			$("#phone").css("top", "14px").css("font-size", "14px");
	...
	$("#input_email_id").blur(function() {
		...
		if(idVal ==""){
			$(this).css("font-size", "5px");
			$("#email_id").css("top", "14px").css("font-size", "14px");
	...
	$("#input_email_pw").blur(function() {
		var idVal = $(this).val();
		if(idVal ==""){
			$(this).css("font-size", "5px");
			$("#email_pw").css("top", "14px").css("font-size", "14px");
	...
	$(document).on("click", ".btn_agree", function(){
		...
		if(name == ""){
			mname.focus();
			mname.next().text("이름을 입력해 주세요").css("display", "block");
			return false;
		}
		...
		if(mailid == ""){
			mmailid1.focus();
			mmailid1.next().text("필수 정보입니다").css("display", "block");
			return false;
		}else if(mailurl == ""){
			mmailid1.focus();
			mmailid1.next().text("이메일형식이 올바르지 않습니다").css("display", "block");
			return false;
		}else if(!regMail.test(mail)){
			mmailid1.select();
			mmailid1.next().text("E-MAIL형식이 올바르시 않습니다 ").css("display", "block");
			return false;
		}
		$("#email").val(mail);
		form.submit();
...
<body>
	<div id="body_div">
		<header>
			...
			<div id="header_div">
				<a href="${path}/index">
					<img alt="may"src="${path}/img/logo_002.png">
				</a>
			</div>
			<h2 id="mm">회원 정보 수정</h2>
		</header>
		
		<section>
		
		<form action="${path}/member/mypage" method="POST" name="frm_mem" id="frm_mem">
		<!-- name값과 id값은 똑같이 해주는게 좋다  -->

		<div id="insert_id" class="insert_div">
			<label for="input_id" class="insert_label" id="label_id">
				ID
			</label>
			<input type="text" id="input_id" name="id" class="input_class" value="${sessionScope.loginUser.id}" readonly="readonly" >
		...
		<div id="insert_name" class="insert_div">
			...
			<input type="text" id="input_name" name="name" class="input_class" value="${sessionScope.loginUser.name}">
		...
		<div id="insert_ph" class="insert_div">
			...
			<input type="text" id="input_ph" name="phone" class="input_class" maxlength=11 value="${sessionScope.loginUser.phone}">
			<span class="error">필수 정보입니다 </span>
		</div>
		...
		<div id="insert_em" class="insert_div">
			...
			<div>
				...
				<input type="text" id="input_email_id" name="input_email_id" id="email" class="email input_class" value="${sessionScope.loginUser.email}">
				<span class="error">필수 정보입니다 </span>
			</div>
			...
			<div id="email2">
				...
				<input type="text" id="input_email_pw" name="input_email_pw" class="email" id="mail"  value="${sessionScope.loginUser.email}">
				<span class="error">필수 정보입니다 </span>
			...
			<div><a href="#" id="btn_in" class ="btn_agree">회원 수정</a></div>
